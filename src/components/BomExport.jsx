  ...
  return (
    <div className="ph-wrap">
      ...
      <div className="ph-section-title">
        ประวัติการอัพเดท
        {history.length > 0 && <span className="ph-count">{history.length} รายการ</span>}
      </div>
      {history.length === 0 ? (
        <div className="ph-empty">ยังไม่มีประวัติการอัพเดท</div>
      ) : (
              ...
              <div className="ph-history-info">
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                  ...
                  <span className={`ph-event-badge ph-event-badge--${h.event_type ?? 'added'}`}>
                    {h.event_type === 'added'          ? '+ เพิ่มใหม่'
                    : h.event_type === 'revised'        ? '✎ อัพเดท Document'
                    : h.event_type === 'image_replaced' ? '🖼 เปลี่ยนรูป'
                    : h.event_type}
                  </span>
                </div>
