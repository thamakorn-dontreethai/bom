...
function buildTree(items) {
  ...
  const roots = []
  const root1 = items.find(i => i.level === 1 && i.status !== 'revised_out')
  items.forEach(i => {
  ...
  return (
    <div className="tree-wrap">
      ...
      <div className="tree-stats-bar">
        ...
        <div style={{ flex: 1 }} />
        <button className="tree-ctrl-btn" onClick={() => setCollapsed(new Set())}>Expand All</button>
        <button className="tree-ctrl-btn" onClick={() => setCollapsed(collectParentIds(roots))}>Collapse All</button>
      </div>
      ...
      {selectedPart && (
        <div className="tree-detail">
          <div className="tree-detail-title">Selected Part Details</div>
          {[
